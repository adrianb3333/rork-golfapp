...
export default function WedData() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setDrills(results.filter(r => r.category === 'Wedges'));
        console.log('[WedData] Loaded wedge drills:', results.filter(r => r.category === 'Wedges').length);
      } catch (e) {
        console.log('[WedData] Error loading drills:', e);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (drillNames.length === 0) {
    return (
      <View style={styles.container}>
        <Text style={styles.emptyText}>No wedge drill data yet</Text>
        <Text style={styles.emptySubtext}>Complete wedge drills to see stats here</Text>
      </View>
    );
  }
