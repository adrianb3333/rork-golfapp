...
export default function PutData() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setDrills(results.filter(r => r.category === 'Putting'));
        console.log('[PutData] Loaded putting drills:', results.filter(r => r.category === 'Putting').length);
      } catch (e) {
        console.log('[PutData] Error loading drills:', e);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (drillNames.length === 0) {
    return (
      <View style={styles.container}>
        <Text style={styles.emptyText}>No putting drill data yet</Text>
        <Text style={styles.emptySubtext}>Complete putting drills to see stats here</Text>
      </View>
    );
  }
