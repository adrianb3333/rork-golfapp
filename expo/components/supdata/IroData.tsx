...
export default function IroData() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setDrills(results.filter(r => r.category === 'Irons'));
        console.log('[IroData] Loaded iron drills:', results.filter(r => r.category === 'Irons').length);
      } catch (e) {
        console.log('[IroData] Error loading drills:', e);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (drillNames.length === 0) {
    return (
      <View style={styles.container}>
        <Text style={styles.emptyText}>No iron drill data yet</Text>
        <Text style={styles.emptySubtext}>Complete iron drills to see stats here</Text>
      </View>
    );
  }
