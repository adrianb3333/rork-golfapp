...
} from "@/services/golfCalculations";
        ...
        >
          <View style={styles.compassWrapper}>
            {loading ? (
              ...
              <WindCompass
                windDirectionFromAPI={weather?.windDeg || 0}
                deviceHeading={deviceHeading}
              />
            )}
          </View>

          <BallFlightToggle selected={ballFlight} onSelect={setBallFlight} />
          ...
          <View style={styles.statsContainer}>
            <View style={styles.statsRow}>
              <StatCard
                label="Wind💨"
                value={weather ? `${weather.windMs} m/s` : "-- m/s"}
              />
              <StatCard
                label="Sea Level🏔️"
                value={weather ? `${weather.seaLevel}m` : "--m"}
              />
              <StatCard
                label="Temp☀️"
            ...
            <View style={styles.statsRow}>
              ...
              <StatCard
                label="From"
                value={weather ? `${weather.windDeg}°` : "--°"}
              />
              <StatCard
                label="Gust"
                value={weather ? `${weather.gustMs} m/s` : "-- m/s"}
              />
            </View>
            ...
            <View style={styles.statsRow}>
              <StatCard
                ...
                value={
                  liveWind
                    ? `${liveWind.cross > 0 ? "+" : ""}${liveWind.cross} m/s`
                    : "-- m/s"
                }
              />
              <StatCard
                ...
                value={
                  liveWind
                    ? `${liveWind.headTail > 0 ? "+" : ""}${liveWind.headTail} m/s`
                    : "-- m/s"
                }
              />
              <StatCard
