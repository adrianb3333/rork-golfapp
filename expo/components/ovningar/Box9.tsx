...
const LiquidGlassCard: React.FC<LiquidGlassCardProps> = ({
  ...
  const liveTotalMade = historyForStep.totalMade + (selectedIndex === -1 ? 0 : currentMade);
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40 }}>
          <View style={styles.gridContainer}>
            {Array(BUTTON_COUNT).fill(0).map((_, i) => {
              ...
              return (
                <View key={`${step}-${i}`} style={styles.gridItem}>
                  <Pressable 
          ...
          <View style={styles.modalBackground}>
            <View style={styles.modalContainer}>
                <ScrollView contentContainerStyle={{ padding: 24 }}>
                    <Text style={[styles.modalTitle, { color: currentColor }]}>9 Box Drill</Text>
                    <Text style={styles.modalText}>Improve consistency across the 9 Box grid at {header} range.</Text>
...
const ResultsCard: React.FC<{ scores: number[]; avgs: number[]; onBack: () => void; onRestart: () => void }> = ({ scores, avgs, onBack, onRestart }) => {
  const labels = ['1 Meter', '3 Meter', '5 Meter'];
  return (
    <SafeAreaView style={styles.fullScreenContainer}>
      <View style={styles.innerContent}>
        <Pressable onPress={onBack} style={styles.internalBackButton}>
        ...
        <Text style={{ color: COLORS.textDim, marginBottom: 32, fontSize: 16 }}>Great 9 Box session today.</Text>
        ...
        <View style={styles.avgCard}>
          <Text style={styles.avgCardTitle}>9 Box Average</Text>
          <View style={styles.resultsAvgRow}>
            {labels.map((label, i) => (
              <View key={i} style={{ alignItems: 'center' }}>
                <Text style={styles.miniLabel}>{label.split(' ')[0]}</Text> 
...
export default function Box9({ onBack, drillName = "9 Box" }: { onBack: () => void, drillName?: string }) {
      ...
      <View style={styles.startOverlay}>
        <Pressable onPress={() => setIsStarted(true)} style={styles.startButtonContainer}>
          <View style={[styles.startDrillBorder, { borderColor: COLORS.blue }]}>
            <Text style={[styles.startDrillText, { color: COLORS.blue }]}>Start 9 Box</Text>
          </View>
  ...
  return (
    <View style={{ flex: 1, backgroundColor: COLORS.bg }}>
      {step < 3 ? (
        <LiquidGlassCard 
          key={`step-${step}-${roundCount}`} // Key forces reset when step or round resets
          header={headers[step]} 
          step={step} 
          onNext={(score, made) => {
