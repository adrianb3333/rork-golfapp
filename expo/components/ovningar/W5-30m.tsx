...
const getColor = (step: number) => {
  if (step < 2) return COLORS.blue; 
  if (step < 4) return COLORS.yellow; 
  return COLORS.red; 
};

const getGradientColors = (step: number) => {
  if (step < 2) return { start: '#ade8f4', end: COLORS.blue };
  if (step < 4) return { start: '#faedcd', end: COLORS.yellow };
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40 }}>
          ...
          <View style={[styles.loaderRow, { marginBottom: 30 }]}>
            ...
            <View style={styles.loaderLabels}>
              ...
              <Text style={{ color: COLORS.textDim, fontSize: 14, marginBottom: 12 }}>Session {roundCount}</Text>
              <Pressable 
          ...
          <View style={styles.modalBackground}>
            <View style={styles.modalContainer}>
                <ScrollView contentContainerStyle={{ padding: 24 }}>
                    <Text style={[styles.modalTitle, { color: currentColor }]}>Wedge 5-30m</Text>
                    <Text style={styles.modalText}>Master distance control by hitting 10 shots at each 5-meter increment. Focus on landing spot precision.</Text>
...
const ResultsCard: React.FC<{ scores: number[]; avgs: number[]; onBack: () => void; onRestart: () => void }> = ({ scores, avgs, onBack, onRestart }) => {
  const labels = ['5 Meter', '10 Meter', '15 Meter', '20 Meter', '25 Meter', '30 Meter'];
  return (
    <SafeAreaView style={styles.fullScreenContainer}>
      <View style={styles.innerContent}>
        <Pressable onPress={onBack} style={styles.internalBackButton}>
        ...
        <Text style={{ color: COLORS.textDim, marginBottom: 32, fontSize: 16 }}>Great session today.</Text>
        ...
        <ScrollView showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.avgCard}>
            <Text style={styles.avgCardTitle}>Performance Averages</Text>
            <View style={styles.resultsAvgRow}>
              {labels.map((label, i) => (
                <View key={i} style={{ alignItems: 'center', width: '33%', marginBottom: 15 }}>
                  <Text style={styles.miniLabel}>{label.split(' ')[0]}M</Text> 
                  <Text style={{ color: getColor(i), fontSize: 22, fontWeight: '800' }}>
                    {avgs[i].toFixed(1)}
                  </Text>
                </View>
              ))}
...
export default function Wedge5_30({ onBack, drillName = "Wedge 5-30m" }: { onBack: () => void, drillName?: string }) {
  const [step, setStep] = useState(0);
  const [scores, setScores] = useState<number[]>([]);
  const [isStarted, setIsStarted] = useState(false);
  const [roundCount, setRoundCount] = useState(1);
  const headers = ['5 Meter', '10 Meter', '15 Meter', '20 Meter', '25 Meter', '30 Meter'];
  const [history, setHistory] = useState(Array(6).fill({ totalMade: 0, rounds: 0 }));
  ...
  const saveToSupabase = async (finalMade: number) => {
    try {
      ...
      console.error("Error saving Wedge Drill:", err);
    }
  };
      ...
      <View style={styles.startOverlay}>
        <Pressable onPress={() => setIsStarted(true)} style={styles.startButtonContainer}>
          <View style={[styles.startDrillBorder, { borderColor: COLORS.blue }]}>
            <Text style={[styles.startDrillText, { color: COLORS.blue }]}>Start Wedge 5-30</Text>
          </View>
          <Text style={[styles.dateText, { color: COLORS.textDim }]}>{new Date().toDateString()}</Text>
        </Pressable>
      </View>
    );
  }
...
const styles = StyleSheet.create({
  ...
  resultsAvgRow: { flexDirection: 'row', justifyContent: 'flex-start', flexWrap: 'wrap' },
