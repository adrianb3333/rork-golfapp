...
const getGradientColors = (step: number) => {
  if (step === 0) return { start: '#caf0f8', end: COLORS.blue };
  if (step === 1) return { start: '#ffffff', end: COLORS.yellow };
  return { start: '#ff4d6d', end: COLORS.red };
};
          ...
          <View style={styles.modalBackground}>
            <View style={styles.modalContainer}>
                <ScrollView contentContainerStyle={{ padding: 24 }}>
                    <Text style={[styles.modalTitle, { color: currentColor }]}>Area Towel Drill</Text>
                    <Text style={styles.modalText}>Precision landing practice. Focus on landing the ball exactly on the towel at {header} distance.</Text>
...
const ResultsCard: React.FC<{ scores: number[]; avgs: number[]; onBack: () => void; onRestart: () => void }> = ({ scores, avgs, onBack, onRestart }) => {
  const labels = ['5 Meter', '10 Meter', '15 Meter'];
  return (
    <SafeAreaView style={styles.fullScreenContainer}>
      <View style={styles.innerContent}>
        <Pressable onPress={onBack} style={styles.internalBackButton}>
        ...
        <Text style={{ color: COLORS.textDim, marginBottom: 32, fontSize: 16 }}>Excellent landing control.</Text>
...
export default function AreaTowel({ onBack, drillName = "Area Towel" }: { onBack: () => void, drillName?: string }) {
  ...
  const headers = ['5 Meter', '10 Meter', '15 Meter'];
  ...
  const saveToSupabase = async (finalMade: number) => {
    try {
      ...
      console.error("Error saving Area Towel Drill:", err);
    }
  };
      ...
      <View style={styles.startOverlay}>
        <Pressable onPress={() => setIsStarted(true)} style={styles.startButtonContainer}>
          <View style={[styles.startDrillBorder, { borderColor: COLORS.blue }]}>
            <Text style={[styles.startDrillText, { color: COLORS.blue }]}>Start Towel Drill</Text>
          </View>
