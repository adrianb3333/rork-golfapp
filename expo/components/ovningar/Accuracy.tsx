          ...
          <View style={styles.modalBackground}>
            <View style={styles.modalContainer}>
                <ScrollView contentContainerStyle={{ padding: 24 }}>
                    <Text style={[styles.modalTitle, { color: currentColor }]}>Accuracy Drill</Text>
                    <Text style={styles.modalText}>Improve accuracy and consistency with your {header}.</Text>
...
const ResultsCard: React.FC<{ scores: number[]; avgs: number[]; labels: string[]; onBack: () => void; onRestart: () => void }> = ({ scores, avgs, labels, onBack, onRestart }) => {
  return (
    <SafeAreaView style={styles.fullScreenContainer}>
      <View style={styles.innerContent}>
        <Pressable onPress={onBack} style={styles.internalBackButton}>
...
export default function AccuracyDrill({ onBack, drillName = "Accuracy" }: { onBack: () => void, drillName?: string }) {
  ...
  const headers = ["5 Wood, 235m", "3 Wood, 265m", "Driver, 285m"];

  const saveToSupabase = async (finalMade: number, stepIndex: number) => {
    try {
      ...
      const { error } = await supabase
        ...
        .insert([
          { 
            user_id: user.id, 
            drill_name: `${drillName} - ${headers[stepIndex]}`, 
            score: finalMade 
          },
        ]);
