    ...
    <SafeAreaView style={styles.fullScreenContainer}>
      <View style={styles.innerContent}>
        <Pressable onPress={onBack} style={styles.internalBackButton}>
          <Text style={styles.internalBackText}>← Back to My Routine</Text>
        </Pressable>
          ...
          <View style={styles.modalBackground}>
            <View style={styles.modalContainer}>
                <ScrollView contentContainerStyle={{ padding: 24 }}>
                    <Text style={[styles.modalTitle, { color: currentColor }]}>My Routine</Text>
                    <Text style={styles.modalText}>Improve consistency and strike quality with your {header}.</Text>
...
const ResultsCard: React.FC<{ scores: number[]; avgs: number[]; onBack: () => void; onRestart: () => void }> = ({ scores, avgs, onBack, onRestart }) => {
  const labels = ['9 iron', '7 iron', '5 iron'];
  return (
    <SafeAreaView style={styles.fullScreenContainer}>
      <View style={styles.innerContent}>
        <Pressable onPress={onBack} style={styles.internalBackButton}>
        ...
        <Text style={[styles.headerText, { marginBottom: 8, color: '#fff' }]}>Routine Summary</Text>
          ...
          <View style={styles.resultsAvgRow}>
            {labels.map((label, i) => (
              <View key={i} style={{ alignItems: 'center' }}>
                ...
                <Text style={{ color: 'rgba(255,255,255,0.2)', fontSize: 10, fontWeight: '700' }}>HIT</Text>
              </View>
            ))}
          </View>
        </View>
...
export default function MyRoutine({ onBack, drillName = "My Routine" }: { onBack: () => void, drillName?: string }) {
  ...
  const headers = ['9 iron', '7 iron', '5 iron'];
      ...
      <View style={styles.startOverlay}>
        <Pressable onPress={() => setIsStarted(true)} style={styles.startButtonContainer}>
          <View style={[styles.startDrillBorder, { borderColor: COLORS.blue }]}>
            <Text style={[styles.startDrillText, { color: COLORS.blue }]}>Start Routine</Text>
          </View>
...
const styles = StyleSheet.create({
  ...
  startDrillBorder: { borderWidth: 1, paddingVertical: 12, paddingHorizontal: 32, borderRadius: 12 },
  startDrillText: { fontSize: 18, fontWeight: '700' as const },
  dateText: { fontSize: 14, marginTop: 12 },
});
